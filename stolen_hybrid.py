...
class HybridUCB:
	...
	def update(self, reward):
		if reward == -1:
			...
			self.theta = self.AaIba - np.dot(self.AaIBa,
											 self.beta)  # self.AaI[article].dot(self.ba[article] - self.Ba[article].dot(self.beta))
	...
	def recommend(self, timestamp, user_features, articles):
		...
		A0IBaTAaIxa_tmp = np.transpose(
			np.dot(np.transpose(np.dot(self.BaTAaI[index], self.xa), (0, 2, 1)), np.transpose(self.A0I)), (0, 2, 1))
		...
		sa_1_tmp = np.sum(
			za_tmp.reshape(article_len, self.k, 1, 1) * A0Iza_diff_2A0IBaTAaIxa_tmp.reshape(article_len, self.k, 1, 1),
			-3)

		# np.dot(AaIBa_tmp, A0IBaTAaIxa_tmp)
		AaIxa_add_AaIBaA0IBaTAaIxa_tmp = np.dot(self.AaI[index], self.xa) + np.sum(
			np.transpose(self.AaIBa[index], (0, 2, 1)).reshape(article_len, self.k, self.d,
															   1) * A0IBaTAaIxa_tmp.reshape(article_len, self.k, 1, 1),
			-3)
...
def recommend(timestamp, user_features, articles):
	...
	if t < break_point:
		...
		return HybridUCBObj.recommend(timestamp, user_features, articles)
